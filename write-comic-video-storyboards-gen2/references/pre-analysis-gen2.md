...
- **补格**: add only necessary establishing, connective, reaction, or result material.
...
- **Action**: an act whose process or result changes a prop, body, location, or story state. Expand setup → execution → response → result → endpoint as useful.
...
| Crop↔original | Location | Characters | Visible action/pose | Dialogue | Props/hands | Enter→exit state | 保拆补并 | State/Action + reaction tag | Relationship/structure | Reproduction anchors | Ambiguity |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
Time each speaker naturally before segmentation. Split at question/answer, interruption, completed action, or reaction. List listener actions while another person speaks.
...
- structural line-art and rendering packages only for recurring spatially complex locations; never output an overhead plan;
