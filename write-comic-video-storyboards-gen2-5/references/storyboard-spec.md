...
Apply a **per-block background gate** before accepting the draft. For every numbered `分镜` and every `战斗段`, first classify the visible backing internally as physical setting, graphic field, or a close surface/subject filling the frame. Then verify that the generatable prose itself positively names what is visible, including useful color, layer, texture, and motion. A tight insert may say `深酒红内衬与胡桃木盒壁铺满背景`; a graphic close-up may say `暖象牙色向淡天蓝渐变，深蓝灰放射线从握点外扩`. Never output the classification, source comparison, or absence explanation. Phrases such as `无实体背景`, `无独立背景`, `原格没有实景`, `盒外环境不可见`, `不补画树林`, and `不加入环境` fail this gate. Rewrite them as direct positive imagery. The validator supplies a lexical safety net, but the image review remains authoritative.
...
Apply `gen2-5-cinematic-extension.md` §4 for per-shot performance density, enacted long-take admission, and serial/concurrent timing. Speech and acting must both be estimated; the detailed gate is maintained there.
